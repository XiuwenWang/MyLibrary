...
public class DeviceUtils {
    ...
    public static String getIMEI(Context context) {
        ...
        if ( cn.finalteam.toolsfinal.StringUtils.isEmpty(imei) ) {
            imei = "";
        }
    ...
    public static String getMac(Context context) {
        ...
        if ( cn.finalteam.toolsfinal.StringUtils.isEmpty(mac) ) {
            mac = "";
        }
        return mac;
    }
    ...
    public static String getUDID(Context context) {
        ...
        if (cn.finalteam.toolsfinal.StringUtils.isEmpty(udid) || udid.equals("9774d56d682e549c")
                || udid.length() < 15) {
            SecureRandom random = new SecureRandom();
            udid = new BigInteger(64, random).toString(16);
        }

        if ( cn.finalteam.toolsfinal.StringUtils.isEmpty(udid) ) {
            udid = "";
        }
     ...
     * @param alpha
     */
    public static void setActivityAlpha(Activity context, float alpha) {
        final WindowManager.LayoutParams params = context.getWindow().getAttributes();//创建当前界面的一个参数对象
        params.alpha = alpha;//设置参数的透明度为0.8，透明度取值为0~1，1为完全不透明，0为完全透明，因为android中默认的屏幕颜色都是纯黑色的，所以如果设置为1，那么背景将都是黑色，设置为0，背景显示我们的当前界面
        if (params.alpha == 1) {
        ...
        context.getWindow().setAttributes(params);//把该参数对象设置进当前界面中
    }
}
